...
export function Hero({ hero }: HeroProps) {
  return (
    <section id="hero" className="relative min-h-screen flex items-center overflow-hidden pt-24">
      ...
      <div className="absolute top-20 right-0 w-[600px] h-[600px] rounded-full bg-[var(--primary)]/5 blur-3xl" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full bg-[var(--secondary)]/5 blur-3xl" />

      <div className="max-w-7xl mx-auto px-4 md:px-6 w-full">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <motion.div variants={containerVariants} initial="hidden" animate="visible">
            <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[var(--primary)]/10 border border-[var(--primary)]/15 text-xs font-semibold text-[var(--primary)] mb-6">
              <span className="w-1.5 h-1.5 rounded-full bg-[var(--primary)] animate-pulse" />
              Now accepting new patients
            </motion.div>

            <motion.h1 variants={itemVariants} className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold text-[var(--text)] leading-[1.05] tracking-tight mb-6 text-balance">
              {hero.title.map((part, idx) => (
                <span key={idx} className={part.type === 'highlight' ? 'text-[var(--primary)]' : ''}>
                  {part.text}{' '}
                </span>
              ))}
            </motion.h1>
        ...
        <motion.div
          ...
          className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-12 md:mt-16 p-6 md:p-8 rounded-2xl bg-[var(--bg-card)] border border-[var(--border)] shadow-sm"
        >
          {hero.stats.map((stat) => (
            <AnimatedCounter
              key={stat.label}
